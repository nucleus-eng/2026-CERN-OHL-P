...
GAIN_COLUMN_NAME = "Read"
DEFAULT_FIT_FUNCTION = "sigmoid"
...
#     # massage Time
#     data["TimeDelta"] = data[TIME_COLUMN_NAME] - data[TIME_COLUMN_NAME].min()
...
#     data[DATA_COLUMN_NAME] = data["CalculatedFlux"]
#     data[DATA_COLUMN_NAME].fillna(data[data["CalculatedFlux"].isna()]["OpticalDensity"], inplace=True)
...
def read_biotek_cdk(data_file: DataFile, sep="\t") -> pd.DataFrame:
    ...
    for header, section in zip(blocks[::2], blocks[1::2]):
        ...
        if data_protocol == "Pierce660":
            ...
            data[GAIN_COLUMN_NAME] = header.split(":")[1]
...
def read_cytation(data_file: DataFile, sep="\t") -> pd.DataFrame:
    ...
    for name, r in reads.items():
        ...
        r = r.melt(id_vars=[TIME_COLUMN_NAME, "T°"], var_name="Well", value_name=DATA_COLUMN_NAME)
        ...
        r[GAIN_COLUMN_NAME] = name
        r["Ex"] = r[GAIN_COLUMN_NAME].str.extract(r"(\d+),\d+").astype(int)
        r["Em"] = r[GAIN_COLUMN_NAME].str.extract(r"\d+,(\d+)").astype(int)
...
def read_envision(data_file: DataFile) -> pd.DataFrame:
    ...
    data[GAIN_COLUMN_NAME] = data["Operation"]
...
def blank_data(data: pd.DataFrame, blank_type="Blank"):
    ...
    blank = (
        ...
        .groupby([TIME_COLUMN_NAME, GAIN_COLUMN_NAME])[DATA_COLUMN_NAME]
        .mean()
    )
    data = data.merge(
        blank, on=[TIME_COLUMN_NAME, GAIN_COLUMN_NAME], suffixes=("", "_blank"), how="left"
    )
    ...
    data["Data_unblanked"] = data[DATA_COLUMN_NAME].copy()
    data[DATA_COLUMN_NAME] = data[DATA_COLUMN_NAME] - data["Data_blank"]
...
def plot_curves_by_name(
    ...
    if "row" not in kwargs and data[GAIN_COLUMN_NAME].unique().size > 1:
        kwargs["row"] = GAIN_COLUMN_NAME

    g = plot_curves(data=data, x=TIME_COLUMN_NAME, y=DATA_COLUMN_NAME, hue="Name", **kwargs)
...
def plot_curves(
    ...
    **kwargs,
    ...
    Args:
        ...
        y (str, optional): Column name to use for y-axis. Defaults to DATA_COLUMN_NAME.
...
def find_steady_state_for_well(well, window=3, data_col = DATA_COLUMN_NAME):
    ...
    return pd.Series(
        {f"{TIME_COLUMN_NAME}_steadystate": ss_time, f"{data_col}_steadystate": ss_level}
    )


def find_steady_state(
    data: pd.DataFrame, group_by=["Name", GAIN_COLUMN_NAME], window=3, data_col = DATA_COLUMN_NAME
) -> pd.DataFrame:
    """
    Find the steady state of the DATA_COLUMN_NAME column in the provided data DataFrame.
    ...
    """
    if data[GAIN_COLUMN_NAME].unique().size > 1 and GAIN_COLUMN_NAME not in group_by:
        log.warning(
    ...
    result = data.groupby(group_by, sort=False).apply(
        find_steady_state_for_well, data_col=data_col
    )
    return result
...
def _data_mean(data, window=3, data_col =DATA_COLUMN_NAME) -> pd.DataFrame:
    data_mean = data.groupby(TIME_COLUMN_NAME, as_index=False)[data_col].mean()
    data_mean[data_col] = data_mean[data_col].rolling(window, min_periods=1).mean()

    return data_mean


def kinetic_analysis_per_well(
    data: pd.DataFrame, data_column=DATA_COLUMN_NAME, fit_function_name=DEFAULT_FIT_FUNCTION
) -> pd.Series:
    if data[data_column].dropna().empty:
        log.warning(f"{data['Well'].iloc[0]} has no valid steady state")
    ...
    data = data.loc[data[TIME_COLUMN_NAME] <= steadystate[f"{TIME_COLUMN_NAME}_steadystate"]]
    time = data[TIME_COLUMN_NAME].dt.total_seconds()
    ...
    # TODO: Test this janky estimation code, and make sure it works across
    # the range of conditions we see in the data.
    data_mean_v = data_mean[DATA_COLUMN_NAME].rolling(3).mean().diff()
    timestep = data_mean[TIME_COLUMN_NAME].dt.total_seconds().sort_values().diff().mean()

    k_initial = data_mean_v.max() / timestep / L_initial * 4
    x0_initial = data_mean[TIME_COLUMN_NAME].dt.total_seconds().loc[data_mean_v.idxmax()]
    ...
    v = (
        ...
        / data_mean[TIME_COLUMN_NAME].dt.total_seconds().diff()
    )
    log.debug(f"V = {v.shape}")

    maxV = v.max()
    maxV_d = data_mean.loc[v.idxmax(), data_column]
    maxV_time = data_mean.loc[v.idxmax(), TIME_COLUMN_NAME]
    ...
    # ss_time = data.loc[(data[data_column] > decile_upper).idxmax(), TIME_COLUMN_NAME]
    # ss_d = data.loc[
    ...
    kinetics = {
        ...
        ("Velocity", "Max"): maxV,
        ("Lag", TIME_COLUMN_NAME): pd.to_timedelta(lag, unit="s"),
        ("Lag", DATA_COLUMN_NAME): lag_data,
        # f"{data_column}_growth_s": growth_s,
        (STEADY_STATE, TIME_COLUMN_NAME): steadystate[f"{TIME_COLUMN_NAME}_steadystate"],
    ...
    if fit_function_name == "sigmoid_drift":
        ...
        kinetics[STEADY_STATE, data_column] = params[0]
        t_alpha = logistic_time_to_fraction(*params[0:3], alpha=0.95)
        kinetics[STEADY_STATE, TIME_COLUMN_NAME] = pd.to_timedelta(t_alpha, unit="s")
...
def kinetic_analysis(
    data: pd.DataFrame, group_by=["Name"], data_column=DATA_COLUMN_NAME, fit_function_name="sigmoid"
) -> pd.DataFrame:
    if data[GAIN_COLUMN_NAME].unique().size > 1 and GAIN_COLUMN_NAME not in group_by:
        log.warning(
    ...
    kinetics = data.groupby(group_by, sort=False).apply(
        functools.partial(kinetic_analysis_per_well, data_column=data_column, fit_function_name=fit_function_name), include_groups=True
    )
    return kinetics
...
def kinetic_analysis_summary(
    ...
    kinetics_styled = kinetics.style.format(precision=precision).format(
        _format_timedelta, subset=pd.IndexSlice[:, pd.IndexSlice[:, TIME_COLUMN_NAME]]
    )
...
def plot_kinetics_by_well(
    ...
    y: str = DATA_COLUMN_NAME,
    show_data: bool = True,
    show_fit: bool = False,
    show_velocity: bool = False,
    show_mean: bool = False,
    annotate: bool = False,
    fit_function_name=DEFAULT_FIT_FUNCTION,
    **kwargs,
):
    """
    Typical usage:
    ...
    if show_fit:
        ...
        sns.lineplot(
            x=data[TIME_COLUMN_NAME],
            y=fit_function(data[TIME_COLUMN_NAME].dt.total_seconds(), *params),
            linestyle="--",
            color=colors[3],
            # alpha=0.5,
            ax=ax,
    ...
    maxV_y = (
        ...
        * (data[TIME_COLUMN_NAME] - kinetics["Velocity", TIME_COLUMN_NAME]).dt.total_seconds()
        + kinetics["Velocity", DATA_COLUMN_NAME]
    )

    sns.lineplot(
        x=data[TIME_COLUMN_NAME].loc[(maxV_y > 0) & (maxV_y < data[y].max())],
    ...
    maxV_s = kinetics["Velocity", TIME_COLUMN_NAME]
    maxV_d = kinetics["Velocity", DATA_COLUMN_NAME]
    ...
    ss_s = kinetics[STEADY_STATE, TIME_COLUMN_NAME]
    ax.axvline(ss_s, c=colors[3], linestyle="--")
    ax.axhline(kinetics[STEADY_STATE, DATA_COLUMN_NAME], c=colors[3], linestyle="--")
    ...
    if annotate:
        ...
        lag_label = f.format_data(
            timple.timedelta.timedelta2num(kinetics["Lag", TIME_COLUMN_NAME])
        )
        ax.annotate(
            f"$t_{{lag}} =$ {lag_label}",
            (kinetics["Lag", TIME_COLUMN_NAME], kinetics["Lag", DATA_COLUMN_NAME]),
            xytext=(12, 0),
            textcoords="offset points",
            ha="left",
            va="center",
        ...
        ss_label = f.format_data(
            timple.timedelta.timedelta2num(kinetics[STEADY_STATE, TIME_COLUMN_NAME])
        )
        ax.annotate(
            f"$t_{{steady state}} =$ {ss_label}",
            (
                kinetics[STEADY_STATE, TIME_COLUMN_NAME],
                kinetics[STEADY_STATE, DATA_COLUMN_NAME],
            ),
            xytext=(0, -12),
            textcoords="offset points",
            ha="left",
...
def plot_kinetics(
    data: pd.DataFrame,
    group_by=["Name", GAIN_COLUMN_NAME],
    kinetics: pd.DataFrame = None,
    show_data: bool = True,
    show_fit: bool = True,
    show_velocity: bool = False,
    show_mean: bool = True,
    annotate: bool = True,
    fit_function_name: str = DEFAULT_FIT_FUNCTION,
    **kwargs,
):
    if kinetics is None:
        kinetics = kinetic_analysis(data, group_by=group_by, fit_function_name=fit_function_name)
...
def plot_steadystate(
    data: pd.DataFrame, x="Name", show_points=False, show_plot = False,
        fit_function=DEFAULT_FIT_FUNCTION, **kwargs
):
    if fit_function == DEFAULT_FIT_FUNCTION:
        steady_state = find_steady_state(
            data, group_by=["Well", GAIN_COLUMN_NAME, x] #if we put in x here, no need to merge as below.
        ).reset_index()
        ss_name = f"{DATA_COLUMN_NAME}_steadystate"
    else:
        steady_state = kinetic_analysis(data, group_by=["Well", GAIN_COLUMN_NAME, x], fit_function_name=fit_function)
        ss_name = (STEADY_STATE, DATA_COLUMN_NAME)
...
def plot_summary(data: pd.DataFrame):
    ...
    fig, axes = plt.subplots(
        exp_count, 2, figsize=(12, 4 * exp_count), squeeze=False
    )
    for i, (experiment, df) in enumerate(experiments):
        ...
        ax_c.set_ylabel("Fluorescence (a.u.)")

        ax_ss = sns.barplot(
            data=find_steady_state(
                df, group_by=["Name", "Well", GAIN_COLUMN_NAME]
            ).reset_index(),
            x="Name",
            y=f"{DATA_COLUMN_NAME}_steadystate",
            hue="Name",
            ax=axes[i, 1],
        )
        ax_ss.tick_params(axis="x", labelsize="x-small")
        ax_ss.set_xlabel(None)
        ax_ss.set_ylabel(None)
...
def compute_standard_curve(
    ...
    def curve(group):
        slope, intercept, r_value, p_value, std_err = stats.linregress(
            group["Conc/Dil"], group[DATA_COLUMN_NAME]
        )
        return pd.Series(
            {
                "slope": slope,
    ...
    curves = (
        std.groupby(["Type", GAIN_COLUMN_NAME])
...
def compute_concentration(
    ...
    conc["Concentration"] = (conc[DATA_COLUMN_NAME] - conc["intercept"]) / conc["slope"]
...
def plot_standard_curve(data: pd.DataFrame, sc_type="STD", **kwargs):
    ...
    std_data = data.merge(curves, on=["Type", GAIN_COLUMN_NAME])
    std_data["StdCurvePoint"] = (
        std_data["Conc/Dil"] * std_data["slope"] + std_data["intercept"]
    )

    if "col" not in kwargs:
        kwargs["col"] = GAIN_COLUMN_NAME

    if "hue" not in kwargs:
        kwargs["hue"] = GAIN_COLUMN_NAME

    g = sns.FacetGrid(data=std_data, **kwargs)
    g.map(sns.scatterplot, "Conc/Dil", DATA_COLUMN_NAME)
